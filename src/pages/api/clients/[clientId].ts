...
export const GET: APIRoute = async ({ params, request })=>{
    ...
    if( clients.length === 0 ){
        return new Response(
            JSON.stringify( {
                msg: `Client not found with id ${ clientId }`
            }),
            {
                status: 404,
                headers: {
...
export const DELETE: APIRoute = async ({ params, request }) => {
    ...
    if( rowsAffected){
        ...
        return new Response(
            JSON.stringify({ msg: `Delete clientId ${clientId}`}),
            {
                status: 200,
                headers: {
                    'Content-Type': 'application/json',
    ...
    return new Response(
        JSON.stringify({ msg: `Not found clientId ${clientId}`}),
        {
            status: 404,
            headers: {
                'Content-Type': 'application/json',
